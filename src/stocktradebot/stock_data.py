...
class DataFetcher:
    ...
    def get_gold_futures_realtime(symbol: str = "AU0") -> Optional[dict]:
        ...
        try:
            ...
            return {
                ...
                "price": float(row["current_price"]) if pd.notna(row.get("current_price")) else 0,
                "change_pct": float(row.get("change_percent", 0)) if pd.notna(row.get("change_percent")) else 0,
    ...
    @staticmethod
    def get_gold_futures_history(symbol: str = "AU0", days: int = 120) -> Optional[pd.DataFrame]:
        """
        获取黄金期货历史K线数据
    ...
    def get_history(self, symbol: str, item_type: str = "stock", days: int = 120) -> Optional[pd.DataFrame]:
        """获取历史数据（自动判断类型）"""
        if item_type == "futures" or symbol.upper().startswith("AU"):
